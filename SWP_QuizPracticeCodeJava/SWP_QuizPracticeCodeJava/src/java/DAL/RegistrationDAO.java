...
//                + "offset (?-1)*? row fetch next ? rows only
    public ArrayList<Registration> getAllRegistrationWithFilter(String subName, String status, String from, String to) {
        ArrayList<Registration> list = new ArrayList<>();
        ...
        try {
            ...
            if ((status != null || !status.equals("-1")) && (subName == null || subName.equals("0")) && (from == null || from.equals("")) && (to == null || to.equals(""))) {
                stm.setString(1, status);
            } else if ((status == null || status.equals("-1")) && (subName != null || !subName.equals("0")) && (from == null || from.equals("")) && (to == null || to.equals(""))) {
...
//            stm.setInt(4, pagesize);
    ...
    public ArrayList<Registration> getAllRegistrationByUserId(int userId) {
        ArrayList<Registration> list = new ArrayList<>();
        //join subject as s on s.subjectId = r.subjectId join pricepackage as p on p.packageId = r.packageId
        PricePackageDAO p = new PricePackageDAO();
        String sql = "SELECT * FROM dbo.Registration AS r JOIN dbo.[User] AS u ON u.userId = r.userId join pricepackage as p on p.packageId = r.packageId JOIN [subject] AS s ON s.subjectId = p.subjectId where u.userId = ?";
...
//            stm.setInt(4, pagesize);
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Registration> list = dao.getAllRegistrationWithFilter("Introduction to Management Senior", "", "", "");
